...
const CartPage = () => {
  ...
  const handleClick = (grp: number, key: number) => {
    if (!parsedCart || !parsedCart.groups?.[grp]?.items[key]) return; // Guard for missing cart or item
    deleteCart(grp, key);
  };
  /*************  ✨ Windsurf Command ⭐  *************/
  /**
   * Delete a cart item by its group and key.
   * @param {number} grp The group of the item.
   * @param {number} key The key of the item.
   */

  /*******  9bc45b6a-922a-4eb9-8b0e-9b1fdc4fdfcb  *******/
  ...
  const deleteCart = async (grp: number, key: number) => {
    ...
    if (!parsedCart || !parsedCart.groups?.[grp]?.items[key].id) {
      console.error('Invalid cart item');
      return;
    }

    const payload = {
      itemId: parsedCart.groups[grp].items[key].id,
      token: token,
    };
    try {
  ...
  if (!parsedCart || !parsedCart.groups || parsedCart.groups.length === 0) {
    return (
      <>
        <Header />
      ...
      <section className='w-[393px] lg:w-[1200px] mx-auto mt-5 px-4 lg:px-0'>
        <div className='flex flex-col lg:flex-row w-full  justify-between'>
          ...
          <div className='lg:col-span-8 w-full'>
            ...
            {/* Shop Card */}
            {Array.from({ length: parsedCart.groups.length }).map((_, i) => (
              <div
              ...
              >
                <div className='flex items-center gap-4 py-4 border-b border-[#E6E8EC]'>
                  ...
                  />
                  <p>{parsedCart.groups[i].shop.name}</p>
                </div>
                {Array.from({ length: parsedCart.groups[i].items.length }).map(
                  (_, j) => (
                    <div
                      ...
                      <div className='flex flex-col items-end gap-2 min-w-[220px]'>
                        ...
                        <div className='flex items-center gap-4'>
                          <Button
                            ...
                            aria-label='Remove'
                            onClick={() => handleClick(i, j)} // Fixed event handler
                          >
                            <Image
                              src={imgTrash}
                              alt=''
                          ...
                          <div className='h-[48px] px-3 rounded-xl border border-[#D5D7DA] flex items-center gap-4'>
                            ...
                            <span className='font-sfpro-bold'>
                              {parsedCart.groups[i].items[j].qty}
                            </span>
          ...
          <aside className='lg:w-[352px]'>
            <div className=' lg:w-[352px] rounded-2xl border border-[#E6E8EC] bg-white p-4 lg:sticky lg:top-6'>
              ...
              <div className='flex items-center justify-between mb-6'>
                ...
                <span className='font-sfpro-bold'>{`Rp${parsedCart.grandTotal.toLocaleString(
                  'id-ID'
                )}`}</span>
              </div>
              <a href='checkout.html'>
                <button className='w-full h-[48px] rounded-xl bg-[#0A0D12] text-[#FFFFFF] font-sfpro-semi-bold hover:cursor-'>
                  Checkout
                </button>
              </a>
            </div>
